...
class RolloutBuffer(BaseBuffer):
    ...
    def __init__(
        ...
        gamma: float = 0.99,
        n_envs: int = 1,
        policy_mem_dim: int = 64,
    ):

        super(RolloutBuffer, self).__init__(buffer_size, observation_space, action_space, device, n_envs=n_envs)
        self.gae_lambda = gae_lambda
        self.gamma = gamma
        self.policy_mem_dim = policy_mem_dim
    ...
    def reset(self) -> None:
        ...
        self.last_policy_mems = np.zeros((self.buffer_size, self.n_envs, self.policy_mem_dim), dtype=np.float32)
    ...
    def add(
        ...
        self.last_policy_mems[self.pos] = np.array(last_policy_mem).copy()
        self.pos += 1
        if self.pos == self.buffer_size:
            self.full = True
    ...
    def get(self, batch_size: Optional[int] = None) -> Generator[RolloutBufferSamples, None, None]:
        ...
        if not self.generator_ready:
            for tensor in ["observations", "actions", "values", "log_probs", "advantages", "returns", "last_policy_mems"]:
